...
public class AppendixDAO {
	...
	public List<Appendix> findAppexdixByApply(int applyId, String applyType){
		Session session = getSession();
		session.beginTransaction();
		String hql = "from Appendix where applyId = " + applyId + " and applyType = '" + applyType + "' order by appendixId";
		List<Appendix> appendixs = session.createQuery(hql).list();
		session.getTransaction().commit();
		return appendixs;
	}
